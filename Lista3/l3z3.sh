...
do
	for WORD in `echo "$FILE" | xargs svn cat | grep -o -E -h '\w+' -- | sort -u` #różnica względem zadania 2.
	# tutaj przechodzimy dzięki flagom grepa i sort -u tylko po UNIKATOWYCH SŁOWACH (dla danego pliku słowo wystąpi max raz)
	do
		if [ stats[$WORD] ]; then
			((stats[$WORD]++))
		else
